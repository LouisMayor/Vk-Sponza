...
{
	if (SponzaDemoSettings::Instance()->sample_level != local_settings.sample_level)
	{
		if (local_settings.sample_level > 0)
		{
			SponzaDemoSettings::Instance()->SetSampleCount(local_settings.sample_level);
		}
		local_settings.sample_level = SponzaDemoSettings::Instance()->sample_level;
	}

	SponzaDemoSettings::Instance()->SetMSAA(local_settings.use_msaa);
}
